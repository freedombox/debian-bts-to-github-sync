...
def fetch_github_issues_by_repo(github_repo, sync_label):
...
def sync(ghclient, debian_pkg_name, github_repo_name, sync_label):
    ...
    github_repo = ghclient.get_repo(github_repo_name)
...
def main():
    ...
    ghclient = Github(conf['github_api_token'])
    ...
    for repo_conf in conf['repositories']:
        ...
        sync(ghclient, debian_pkg_name, github_repo_name, sync_label)
